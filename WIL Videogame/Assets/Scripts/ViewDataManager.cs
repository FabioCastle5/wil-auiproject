...
public class ViewDataManager : MonoBehaviour {
	...
	public void BuildCircuit () {
		...
		circuitInstance.GetComponent<ViewCircuitManager> ().Reset ();
	...
	*/
	...
	void DrawStep(int x0, int y0, int x1, int y1, ref float x, ref float y) {
		...
		circuitInstance.GetComponent<ViewCircuitManager> ().AddTile (x, y, tiles[chosenTileIndex]);
	...
	void DrawCurve(int x0, int y0, int x1, int y1, int x2, int y2, ref float x, ref float y) {
		...
		if (y2 == y0 && y0 != y1) {
			...
			if (y1 > y0) {
				if (x0 > x2) {
					circuitInstance.GetComponent<ViewCircuitManager> ().AddTile (x, y, tiles [5]);
					x -= horizontalOffset;
					circuitInstance.GetComponent<ViewCircuitManager> ().AddTile (x, y, tiles [4]);
					y -= verticalOffset;
				} else if (x0 < x2) {
					circuitInstance.GetComponent<ViewCircuitManager> ().AddTile (x, y, tiles [4]);
					x += horizontalOffset;
				...
				if (x0 > x2) {
					circuitInstance.GetComponent<ViewCircuitManager> ().AddTile (x, y, tiles [6]);
					x -= horizontalOffset;
					circuitInstance.GetComponent<ViewCircuitManager> ().AddTile (x, y, tiles [7]);
					y += verticalOffset;
				} else if (x0 < x2) {
					circuitInstance.GetComponent<ViewCircuitManager> ().AddTile (x, y, tiles [7]);
					x += horizontalOffset;
	...
	void drawFinish(int x0, int y0, int x1, int y1, float x, float y) {
		...
		circuitInstance.GetComponent<ViewCircuitManager> ().DrawFinalFlag (flags [flagIndex]);
	}
}
